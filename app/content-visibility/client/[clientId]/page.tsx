...
// Minimal mock data - only 3 posts instead of many
const MOCK_POSTS: Post[] = [
  {
    ...
    review_text: "Strong engagement from target demographic.",
...
// Lazy load snapshots only when timeline view is active
const getSnapshotData = (postId: string): MetricSnapshot[] => {
...
export default function ClientDetailPage({ params }: { params: { clientId: string } }) {
  ...
  const totalReach = useMemo(() => MOCK_POSTS.reduce((sum, p) => sum + (p.reach_metric || 0), 0), [])
        ...
        <div className="space-y-6">
          {viewType === "timeline" && (
            <div className="space-y-4">
              {MOCK_POSTS.map((post) => {
                const snapshots = getSnapshotData(post.id)
