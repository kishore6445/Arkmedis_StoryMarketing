...
// Lazy load all heavy components to prevent bundling their data on page load
        ...
        <main className="flex-1 ml-64 transition-all duration-300 mt-16 p-8 [@media(max-width:768px)]:ml-20">
          <div className="max-w-7xl mx-auto space-y-8">
            ...
            {currentPhase === "meetings" && (
              <Suspense fallback={<div className="text-gray-400">Loading meetings...</div>}>
                <MeetingsPage />
              </Suspense>
            )}
            
            {currentPhase === "content-tracker" && (
              <Suspense fallback={<div className="text-gray-400">Loading content tracker...</div>}>
                <ContentTrackerPage />
              </Suspense>
            )}
            
            {currentPhase === "content-visibility" && (
              <Suspense fallback={<div className="text-gray-400">Loading content visibility...</div>}>
                <ContentVisibilityPage />
              </Suspense>
            )}
            
            {currentPhase === "manage-clients" && (
              <Suspense fallback={<div className="text-gray-400">Loading clients...</div>}>
